...
def test_run_meta_analysis_experiment_writes_result_file() -> None:
    ...
    assert payload["analysis"]["summary"]["captured_op_count"] == 83
    ...
    assert any(op["op_name"] == "aten.cos.default" for op in payload["analysis"]["ops"])
    assert any(op["op_name"] == "aten.sin.default" for op in payload["analysis"]["ops"])

    with csv_path.open("r", encoding="utf-8", newline="") as handle:
        rows = list(csv.DictReader(handle))

    assert len(rows) == payload["analysis"]["summary"]["captured_op_count"]
    ...
    assert len(op_events) == payload["analysis"]["summary"]["captured_op_count"]
    ...
    assert any(event["name"] == "aten.topk.default" for event in op_events)
    assert any(event["name"] == "aten.silu.default" for event in op_events)
    assert any(event["name"] == "aten.cos.default" for event in op_events)
    assert any(event["name"] == "aten.sin.default" for event in op_events)
